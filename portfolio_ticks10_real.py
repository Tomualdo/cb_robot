...
def forced_rewrite(strategy_file):
    """rewrite strategy file to FALSE value in forced buy key"""
    with open(strategy_file ,'r+') as new_file:
        ...
        new_file.truncate()        
        json.dump(strategy_data,new_file,indent=4)
        print("forced buy rewrited to false")
    
def buy_procedure(i,current_product,data,buys,calc_buy_size,buy_process_number,strategy_data):
    """
    i = index of tick
...
def buy_bid_check(current_product,actual_buy_price):
    # prepare loop for bid check
    print("CHECKING last close {}".format(actual_buy_price))
    for seq in range(10):
        #check best bid
        bid = get_product_bid(current_product)
        print("best bid is {}".format(bid))
        bid = float(bid['bids'][0][0])
        if bid <= actual_buy_price*1.001:
            print("bid {} is OK  --- BUY STARTED".format(bid))
            return True
        else:
...
def strategy(data,strategy_data,current_product):
    try:
        ...
        ooa_sell_ratio = strategy_data['ooa_sell_ratio']
        ooa_max_buys = strategy_data['ooa_max_buys']
        ooa_out_of_bound_sell_ratio = strategy_data['ooa_out_of_bound_sell_ratio']
        ooa_min_buy_time_minutes = strategy_data['ooa_min_buy_time_minutes']
        ...
        print("Calculated size for {} EUR is {} {} | ballance is {} eur | ratio is {}".format(EUR_to_buy_size,calc_buy_size,current_product,actual_ballance,ballance_ratio))
        ...
        for i in range(len(data['close'])):
            ...
            if buys != {}: # {} dict for prtfolio [] list for anal
                for idx in list(buys.keys()):
                    ooa_buy_key_exist = 'ooa_buy' in buys[idx][0]
                    if buys[idx][0]['sell_flag'] == False and ooa_buy_key_exist:
                        if buys[idx][0]['sell_flag'] == False and buys[idx][0]['ooa_buy'] == False:
                            buys_qty += 1
                        if buys[idx][0]['sell_flag'] == False and buys[idx][0]['ooa_buy'] == True:
                            ooa_qty += 1
            ...
            if len(buys.keys())>=1:
                ...
                if sells_not_empty_record != []:
                    for idx in sells_not_empty_record:
                        ooa_buy_key_exist = 'ooa_buy' in buys[idx][0]
                        if ooa_buy_key_exist:
                            if buys[idx][0]['ooa_buy'] == True:
                                sell_out_of_bounds_price_ratio = ooa_out_of_bound_sell_ratio
                            else:
                                sell_out_of_bounds_price_ratio = out_of_bound_sell_ratio
                        else:
                    ...
                    for idx in sells_not_empty_record:
                        ...
                        if not buys[idx][0]['sell_flag'] and actual_sell_price / buys[idx][0]['buy_price'] >= sell_price_ratio:
                            ...
                            for seq in range(10):
                                ...
                                print("best bid is {}".format(bid))
                                bid = float(bid['bids'][0][0])
                                if bid <= actual_sell_price*0.9998:
                                    print("bid {} did not match. try again ...{}".format(bid,seq))
                                    time.sleep(1.5)
                                else:
            ...
            #BUY--------------------------------------------------------------------------------------------------
            elif data['close'][i] < data['lower'][i] and (buys_qty < max_buys or ooa_qty < ooa_max_buys) and min_ballance_buy < actual_ballance or forced_buy:
                if forced_buy:
                    if buy_bid_check(current_product,data['close'][i]):                           
                        print("buy: {} * FORCED * ".format(current_product))
                        there_was_buy_or_sell = buy_procedure(i,current_product,data,buys,calc_buy_size,'8',strategy_data)
                        # rewrite forced buy to false
                        forced_rewrite(strategy_data)
                        break
                ...
                ooa_active = False
                if data['close'][i] > data['C-AVG'][i]: # OVER AVERAGE
                    if ooa_qty < ooa_max_buys: # BUYS QTY IS OK
                        print("Passing OOA buy for {}".format(current_product))
                        ooa_active = True
                        pass
                    else:                       # BUYS QTY IS OVER LIMIT
                        print("CANCELED OOA buy for {} ooa_qty {} > ooa_max_buys {}".format(current_product,ooa_qty,ooa_max_buys))
                        break
                                   
                if buys_qty < max_buys or ooa_active == True: # normal buy
                    print("Passing buy QTY for {}".format(current_product))
                    pass
                else:
                    print("CANCELED NORMAL buy for {} buys_qty {} > max_buys {}".format(current_product,buys_qty,max_buys))
                    break

                print("Passed buy CHECK {}".format(current_product))
                ...
                if len(buys.keys()) >= 1:
                    ...
                    if ooa_active == False:
                        print("OOA not active")
                        # print("len is {}".format(len(buys.keys())))
                        for idx in range(len(buys.keys())):
                            # print(idx)
                            if buys[list(buys.keys())[idx]] != []:
                                ooa_buy_key_exist = 'ooa_buy' in buys[list(buys.keys())[idx]][0]
                                if ooa_buy_key_exist:
                                    if buys[list(buys.keys())[idx]] != [] and buys[list(buys.keys())[idx]][0]['sell_flag']==False and buys[list(buys.keys())[idx]][0]['ooa_buy']==False: 
                    ...
                    # only OOA buys
                        ...
                        for idx in range(len(buys.keys())):
                            ...
                            if buys[list(buys.keys())[idx]] != []:
                                ooa_buy_key_exist = 'ooa_buy' in buys[list(buys.keys())[idx]][0]
                                if ooa_buy_key_exist:
                                    if buys[list(buys.keys())[idx]] != [] and buys[list(buys.keys())[idx]][0]['sell_flag']==False and buys[list(buys.keys())[idx]][0]['ooa_buy']==True: 
                                        # lets track non empty buys time indexes
                                        # check if there it is not already in the list !!!
                                        if list(buys.keys())[idx] not in buys_not_empty_records:
                                            buys_not_empty_records.append(list(buys.keys())[idx])

                    print(buys_not_empty_records)
                    if buys_not_empty_records == []:
                        # BUY if there are no active buys (everything is sold...)
                        logger.warning("4 EMPTY LIST BUY !!!!!!!!!!!!")
                        print("BUY BID CHECK Process...")
                        if buy_bid_check(current_product,data['close'][i]):
                            print("buy: {} SUCSESS -- no buys active ".format(current_product))
                            there_was_buy_or_sell = buy_procedure(i,current_product,data,buys,calc_buy_size,'4',strategy_data)
                        ...
                        if not ooa_active:
                            ...
                            set_minimum_buy_time = 60*ooa_min_buy_time_minutes
                        ...
                        if diff < set_minimum_buy_time and diff != 0 and buys_not_empty_records != []:
                            ...
                            if not ooa_active:
                                ...
                                buy_ratio = ooa_min_buy_time_buy_ratio
                            ...
                            if ratio_result > buy_ratio:
                                # if buy for better ratio increase investment
                                temp_calc_buy_size = calc_buy_size
                                if not ooa_active: # OOA Active
                                    temp_calc_buy_size = calc_buy_size
                                    calc_buy_size = get_calc_minSize(current_product,EUR_to_buy_size*multiply_size) #consider last_buy_price instead EUR_to_buy_size
                                    print("Investment increased from {} size to {} multipler is {}".format(temp_calc_buy_size,calc_buy_size,multiply_size))
                                else:
                                    # ratio result is better than increase price NO OOA
                                    if ratio_result > buy_ratio*1.008: #(base buy_ratio = 1.02 x 1.008 = 1.028)
                                        calc_buy_size = get_calc_minSize(current_product,EUR_to_buy_size*multiply_size)
                                        print("Investment increased from {} size to {} multipler is {}".format(temp_calc_buy_size,calc_buy_size,multiply_size))
                                if buy_bid_check(current_product,data['close'][i]):                           
                                    print("buy: {} SUCSESS -- ratio_result: {} > buy_ratio: {} ".format(current_product,ratio_result,buy_ratio))
                                    there_was_buy_or_sell = buy_procedure(i,current_product,data,buys,calc_buy_size,'1',strategy_data)
                            else: 
                            ...
                            if not ooa_active:
                                ...
                                max_time_better_price = strategy_data['ooa_max_time_better_price']
                            ...
                            if price_ratio < 1 and diff < set_minimum_buy_time+(60*max_time_better_price) and diff != 0:
                                ...
                                if buy_bid_check(current_product,data['close'][i]):
                                    ...
                                    there_was_buy_or_sell = buy_procedure(i,current_product,data,buys,calc_buy_size,'2',strategy_data)
                # No buys yes so lets buy
                # DONT BUY ANYMORE AFTER 1.ST BUY        
                else:
                    if buy_bid_check(current_product,data['close'][i]):
                        print("buy: {} SUCSESS -- first buy ".format(current_product))
                        there_was_buy_or_sell = buy_procedure(i,current_product,data,buys,calc_buy_size,'3',strategy_data)
