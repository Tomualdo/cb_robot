...
def get_orders_done(currency=None):
    """List your current open orders"""
    if currency == None:
        r = requests.get(api_url + 'orders?status=done', auth=auth)
        return r.json()
    else:
        r = requests.get(api_url + 'orders?limit=4&status=done&product_id='+currency, auth=auth)
        return r.json()
